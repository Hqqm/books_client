...
export const $isFirstNameCorrect = $firstNameError.map<boolean>(
  value => value === null
);

export const $lastName = createStore<string>("");
export const $lastNameError = $lastName.map<string | null>(textValidator);
export const $isLastNameCorrect = $lastNameError.map<boolean>(
  value => value === null
);

export const $passwordConfirmation = createStore<string>("");
export const $passwordConfirmationError = $passwordConfirmation.map<
  string | null
>(currentPassword =>
  passwordConfirmation(currentPassword, $password.getState())
);
export const $isPasswordConfirmationCorrect = $passwordConfirmationError.map<
  boolean
>(value => value === null);
...
$lastName.on(
  lastNameChanged.map(e => e.currentTarget.value),
  (_, lastName) => lastName
);
...
export const $isRegisterSubmitedEnabled = combine(
  $isRegFormValid,
  registerUser.pending,
  (isRegFormValid, isUserRegistering) => isRegFormValid && !isUserRegistering
);
